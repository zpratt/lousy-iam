...
describe("ValidateAndFixOrchestrator", () => {
    ...
    describe("executeWithFixed", () => {
        describe("given an auto-fixable permission policy violation", () => {
            it("should return fixedOutput with the fixer-applied document", () => {
                // Arrange
                const violation = buildViolation({
                    rule_id: "LP-045",
                    auto_fixable: true,
                ...
                const fixedAction = `s3:GetObject-${chance.word()}`;
                const permissionValidator = {
                    validate: vi
                        .fn()
                        .mockReturnValueOnce([violation])
                ...
                const fixer = {
                    fixPermissionPolicy: vi.fn().mockImplementation((doc) => ({
                        ...doc,
                        Statement: [
                            {
                                ...doc.Statement[0],
                ...
                const { validation, fixedOutput } =
                    orchestrator.executeWithFixed(buildFormulationOutput());

                // Assert
                expect(validation.valid).toBe(true);
                expect(
                    fixedOutput.roles[0]?.permission_policies[0]
                        ?.policy_document.Statement[0]?.Action,
                ).toEqual([fixedAction]);
            });
        });

        describe("given an auto-fixable trust policy violation", () => {
            it("should return fixedOutput with the fixer-applied trust policy", () => {
                // Arrange
                const violation = buildViolation({
                    rule_id: "LP-030",
                    auto_fixable: true,
                });
                const fixedSid = `FixedSid-${chance.word()}`;
                const permissionValidator = {
                    validate: vi.fn().mockReturnValue([]),
                };
                const trustValidator = {
                ...
                const fixer = {
                    fixPermissionPolicy: vi.fn(),
                    fixTrustPolicy: vi.fn().mockImplementation((doc) => ({
                        ...doc,
                        Statement: [
                            {
                                ...doc.Statement[0],
                ...
                expect(validation.valid).toBe(true);
                expect(
                    fixedOutput.roles[0]?.trust_policy.Statement[0]?.Sid,
                ).toBe(fixedSid);
            });
        });
    });
