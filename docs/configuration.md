...
|-------|------|---------|-------------|
| `account_id` | string or null | `null` | AWS account ID (12 digits). When provided, replaces `${account_id}` placeholder in trust policy OIDC ARNs with the actual value. **Required for the `synthesize` command** when the formulation output contains `${account_id}` placeholders. |
